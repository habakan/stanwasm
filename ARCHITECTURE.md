...
- Keep total wasm payload small after `wasm-opt -Oz` (currently ~534 KB, ~200 KB gzipped, including `console_error_panic_hook` for browser diagnostics).
...
└──────────┬──────────────────────────────────────────────────────┘
           ...
           ▼
┌─────────────────────────────────────────────────────────────────┐
│  stanwasm.wasm  (single bundle, ~534 KB after wasm-opt)          │
│                                                                  │
...
#   └─ wasm-opt (wasm-pack's release default) shrinks the bundle to ~534 KB
