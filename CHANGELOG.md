...
- **The engine moves from tapewasm v0.1.0 to v0.3.1.** It brings the re-roll
  ...
  recording can offer. 0.3.1 also stops a re-rolled loop's backward pass from
  calling out to `exp` and `log` a second time, which halves the host calls a
  gradient makes on a density that uses them.
