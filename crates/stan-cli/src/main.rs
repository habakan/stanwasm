...
//!
//!   stan-cli bench [model]   run a per-path benchmark suite
//!
//! Times each of:
...
fn main() -> anyhow::Result<()> {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some(other) => {
            ...
            eprintln!("usage: stan-cli bench [linear|poisson|all]");
            std::process::exit(2);
        }
        None => {
            println!("stan-cli v{}", env!("CARGO_PKG_VERSION"));
            println!("usage: stan-cli bench [linear|poisson|all]");
            Ok(())
        }
    }
}
...
fn run_bench(filter: Option<&str>) -> anyhow::Result<()> {
    ...
    println!("- AOT µs    = stan-codegen output run via wasmi (Rust → wasm32)");
...
fn stan_autodiff_lgamma(x: f64) -> f64 {
    stan_autodiff::lgamma(x)
}
fn stan_autodiff_digamma(x: f64) -> f64 {
    stan_autodiff::digamma(x)
}
fn stan_autodiff_phi(x: f64) -> f64 {
    stan_autodiff::phi_cdf(x)
}
