...
fn host_wasm_uses_no_wasm_gc() {
    // Read the most recently-built stan-wasm-api artifact, if it exists.
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../target/wasm32-unknown-unknown/release/stan_wasm_api.wasm");
    if !path.exists() {
        eprintln!("skipping: build with `cargo build -p stan-wasm-api --target wasm32-unknown-unknown --release` first");
        return;
    }
    let bytes = std::fs::read(&path).unwrap();
    validates_without_gc(&bytes).expect("stan-wasm-api wasm must not use GC opcodes");
}
