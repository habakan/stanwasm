...
pub fn normal_rng(rng: &mut impl Rng, mu: f64, sigma: f64) -> f64 {
    mu + sigma * Normal::new(0.0, 1.0).unwrap().sample(rng)
}

pub fn exponential_rng(rng: &mut impl Rng, lambda: f64) -> f64 {
    Exp::new(lambda).unwrap().sample(rng)
}

pub fn half_normal_rng(rng: &mut impl Rng, sigma: f64) -> f64 {
    normal_rng(rng, 0.0, sigma).abs()
}

pub fn cauchy_rng(rng: &mut impl Rng, mu: f64, sigma: f64) -> f64 {
    Cauchy::new(mu, sigma).unwrap().sample(rng)
}

pub fn student_t_rng(rng: &mut impl Rng, nu: f64, mu: f64, sigma: f64) -> f64 {
    mu + sigma * StudentT::new(nu).unwrap().sample(rng)
}

pub fn lognormal_rng(rng: &mut impl Rng, mu: f64, sigma: f64) -> f64 {
    LogNormal::new(mu, sigma).unwrap().sample(rng)
}

/// Stan's `gamma(alpha, beta)` is shape/rate; `rand_distr::Gamma` is shape/scale.
pub fn gamma_rng(rng: &mut impl Rng, alpha: f64, beta: f64) -> f64 {
    Gamma::new(alpha, 1.0 / beta).unwrap().sample(rng)
}

pub fn beta_rng(rng: &mut impl Rng, a: f64, b: f64) -> f64 {
    Beta::new(a, b).unwrap().sample(rng)
}

pub fn uniform_rng(rng: &mut impl Rng, lo: f64, hi: f64) -> f64 {
    Uniform::new(lo, hi).unwrap().sample(rng)
}

pub fn bernoulli_rng(rng: &mut impl Rng, theta: f64) -> f64 {
    if Bernoulli::new(theta).unwrap().sample(rng) {
        1.0
    } else {
        0.0
    }
...
pub fn bernoulli_logit_rng(rng: &mut impl Rng, alpha: f64) -> f64 {
    bernoulli_rng(rng, 1.0 / (1.0 + (-alpha).exp()))
}

pub fn poisson_rng(rng: &mut impl Rng, lambda: f64) -> f64 {
    Poisson::new(lambda).unwrap().sample(rng)
}

/// Gamma-Poisson mixture: `neg_binomial_2(mu, phi)` has mean `mu` and
/// variance `mu + mu^2/phi`.
pub fn neg_binomial_2_rng(rng: &mut impl Rng, mu: f64, phi: f64) -> f64 {
    let lambda = Gamma::new(phi, mu / phi).unwrap().sample(rng);
    Poisson::new(lambda).unwrap().sample(rng)
}

/// Independent `Gamma(alpha_i, 1)` draws, normalized.
pub fn dirichlet_rng(rng: &mut impl Rng, alpha: &[f64]) -> Vec<f64> {
...
pub fn multi_normal_cholesky_rng(rng: &mut impl Rng, mu: &[f64], l: &[Vec<f64>]) -> Vec<f64> {
    let k = mu.len();
    let z: Vec<f64> = (0..k).map(|_| normal_rng(rng, 0.0, 1.0)).collect();
    (0..k)
        .map(|i| {
            let mut s = mu[i];
...
/// context is a programming error and panics.
pub fn dispatch(t: &Tape, base: &str, args: &[Val], env: &Env) -> Val {
    let rng_handle = env
        .rng()
        .unwrap_or_else(|| panic!("{base}_rng called outside generated quantities"));
    ...
    match (base, args) {
        ...
        ("gamma", [alpha, beta]) => Val::Num(gamma_rng(&mut *rng, alpha.to_f64(t), beta.to_f64(t))),
        ...
        _ => panic!("unsupported or wrong-arity call: {base}_rng"),
    }
}
